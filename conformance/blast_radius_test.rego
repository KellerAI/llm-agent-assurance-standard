...
# description: |
#   Enumerates every seed relationship in conformance/affects.json (BR-002
#   through BR-011) plus the empty-diff baseline plus the
...
# ---------------------------------------------------------------------------
# BR-011 — editing conformance/affects.json (the manifest itself) is a
# verifiable=true error entry that BLOCKS until its actions are footer-DONE.
# ---------------------------------------------------------------------------
...
test_br011_clears_when_all_actions_done if {
	# conformance/affects.json matches only BR-011 (exact path); declaring both
	# of its required actions DONE clears the verdict.
	result := blast_radius.result with input as _input(
		["conformance/affects.json"],
		{},
		[
...
test_deterministic_same_input_same_verdict if {
	# CLAUDE.md fires BR-005 (severity=error, verifiable=true) → blocked.
	in1 := _input(
		["CLAUDE.md"], {}, [],
	)
...
test_result_carries_rationale if {
	# CLAUDE.md matches BR-005 (verifiable=true error → 1 error) AND AGENTS.md
	# matches BR-004 (warning → 1 warning). 2 changed, 2 fired, 1 error,
	# 1 warning, verdict blocked.
	result := blast_radius.result with input as _input(
		["CLAUDE.md", "AGENTS.md"], {}, [],
	)
	result.rationale == "changed=2 fired=2 errors=1 warnings=1 -> blocked"
}
