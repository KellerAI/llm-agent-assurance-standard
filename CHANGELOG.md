...
- Controlled-language profiles for LAAS decision traces under `docs/laas/profiles/` (minor, additive): a shared rule base (`ste-core.md`) defining twelve core writing rules, a cross-domain hedge ban list, four language-conformance levels (`LC-0`–`LC-3`), a recommended gate policy (advisory at CT0–CT1, warning at CT2, blocking at CT3–CT4), and a mapping showing how controlled language strengthens each of the twelve existing obligations; plus three industry profiles — `banking.md`, `healthcare.md`, `seo-adsense.md` — each with a domain rule set, an approved noun and verb dictionary, a forbidden-terms table, STE-compliant decision-trace field templates, and a worked example. Machine-readable dictionaries derived from each profile are at `docs/laas/profiles/glossary/*.json`. The profiles are informative and adapt ASD-STE100 principles: they define no new obligation, tier, or threshold, and `standard/LAAS.md`, `conformance/laas/data.json`, and `conformance/laas/laas.rego` are unchanged.
