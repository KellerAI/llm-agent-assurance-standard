...
updates and stops there: `.github/dependabot.yml:1-9` declares one ecosystem
(`github-actions`), a weekly schedule, and a `chore` commit prefix
...
or advances five identified cross-discipline gaps — G-02
...
- Implement in a series of reviewable PRs, each paired with a semver-classified
  `CHANGELOG.md` entry: policy, data, test suite, gate workflow, outcome workflow,
  and audit directory as a coherent sequence.
...
- Resolve open questions OQ-1..OQ-6 before the tiers above Observed are exercised
  in anger — in particular OQ-2 (`outcome_signal` → ELO/KoTH mapping) and OQ-3
  (reusable-workflow factoring).
- Document the migration path for already-bootstrapped repos in
  `docs/agents/enforcement.md`: add `audit/trust-dial-state.json`,
  `audit/decision-trace.jsonl`, `.github/workflows/trust-dial-gate.yml`, and
  `.github/workflows/trust-dial-outcome.yml`, or conformance will block.
